...
export class ConfSerialCrudComponent implements AfterViewInit {
  ...
  delete(id:number){
    this.matDialog.open(YesNOComponent,{data:{action:'Borrar',description:`¿Desea borrar la cama ${id}?`},enterAnimationDuration:500}).afterClosed().subscribe((resDialog)=>{
      if (resDialog===true){
          this.confSerialService.delete(id).subscribe({next:_deleted=>{
